...
func buildOutboxEvent(eventType, aggregateType string, aggregateID uint64, routingKey string, payload any) (*model.EventOutbox, error) {
	...
	envelope, err := newOutboxEnvelope(outboxEnvelopeInput{
		EventID:       eventID,
		EventType:     eventType,
...
func marshalEventPayload(envelope outboxEnvelope, payloadJSON []byte) ([]byte, error) {
	envelopeJSON, err := marshalJSONOutboxEnvelope(envelope)
	if err != nil {
		return nil, err
	}
