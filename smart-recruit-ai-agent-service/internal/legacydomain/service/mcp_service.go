...
func (s *MCPService) TestMCPConnection(ctx context.Context, req *pb.TestMCPConnectionRequest) (*pb.TestMCPConnectionResponse, error) {
	...
	log.Info("[domain][mcp] TestMCPConnection started", zap.Int64("server_id", serverID))
	if serverID <= 0 {
		log.Warn("[domain][mcp] TestMCPConnection invalid server_id")
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Warn("[domain][mcp] TestMCPConnection server not found", zap.Int64("server_id", serverID))
		...
		log.Error("[domain][mcp] TestMCPConnection get server failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get MCP server failed")
	}
	...
	if err != nil {
		log.Warn("[domain][mcp] TestMCPConnection create client failed",
	...
	if err != nil {
		log.Warn("[domain][mcp] TestMCPConnection initialize failed",
			zap.Int64("server_id", serverID), zap.Error(err))
		return &pb.TestMCPConnectionResponse{
			Code:    1,
	...
	log.Info("[domain][mcp] TestMCPConnection succeeded",
...
func (s *MCPService) ListMCPTools(ctx context.Context, req *pb.ListMCPToolsRequest) (*pb.ListMCPToolsResponse, error) {
	...
	log := logger.GetRequestLogger(ctx)
	serverID := req.GetServerId()
	log.Info("[domain][mcp] ListMCPTools started", zap.Int64("server_id", serverID))
	if serverID <= 0 {
		log.Warn("[domain][mcp] ListMCPTools invalid server_id")
		return nil, status.Error(codes.InvalidArgument, "server_id is required")
	}
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Warn("[domain][mcp] ListMCPTools server not found", zap.Int64("server_id", serverID))
			return nil, status.Error(codes.NotFound, "MCP server not found")
		}
		log.Error("[domain][mcp] ListMCPTools get server failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get MCP server failed")
	}

	mcpClient, err := s.createClient(server)
	if err != nil {
		log.Error("[domain][mcp] ListMCPTools create client failed", zap.Error(err))
	...
	if err != nil {
		log.Error("[domain][mcp] ListMCPTools initialize failed", zap.Error(err))
	...
	if err != nil {
		log.Error("[domain][mcp] ListMCPTools list tools failed", zap.Error(err))
	...
	log.Info("[domain][mcp] ListMCPTools succeeded",
		zap.Int64("server_id", serverID),
...
func (s *MCPService) CallMCPTool(ctx context.Context, req *pb.CallMCPToolRequest) (*pb.CallMCPToolResponse, error) {
	...
	log.Info("[domain][mcp] CallMCPTool started",
		zap.Int64("server_id", serverID),
		zap.String("tool_name", toolName))
	if serverID <= 0 {
		log.Warn("[domain][mcp] CallMCPTool invalid server_id")
		return nil, status.Error(codes.InvalidArgument, "server_id is required")
	}
	if toolName == "" {
		log.Warn("[domain][mcp] CallMCPTool invalid tool_name")
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Warn("[domain][mcp] CallMCPTool server not found", zap.Int64("server_id", serverID))
			return nil, status.Error(codes.NotFound, "MCP server not found")
		}
		log.Error("[domain][mcp] CallMCPTool get server failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get MCP server failed")
	}

	if server.IsEnabled != 1 {
		log.Warn("[domain][mcp] CallMCPTool server disabled", zap.Int64("server_id", serverID))
	...
	if err != nil {
		log.Warn("[domain][mcp] CallMCPTool invalid args_json", zap.Error(err))
		return nil, status.Error(codes.InvalidArgument, fmt.Sprintf("invalid args_json: %v", err))
	}

	log.Debug("[domain][mcp] CallMCPTool evaluating policy",
		zap.Int64("server_id", serverID),
		zap.String("tool_name", toolName),
	...
	if err != nil {
		log.Error("[domain][mcp] CallMCPTool policy evaluation failed", zap.Error(err))
	...
	if policyEval.Decision != mcpPolicyDecisionAllow {
		...
		log.Warn("[domain][mcp] CallMCPTool policy denied",
	...
	mcpClient, err := s.createClient(server)
	if err != nil {
		log.Error("[domain][mcp] CallMCPTool create client failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "create MCP client failed")
	}
	defer mcpClient.Close()
	...
	if err != nil {
		log.Error("[domain][mcp] CallMCPTool initialize failed", zap.Error(err))
		return nil, status.Error(codes.Internal, fmt.Sprintf("MCP initialize failed: %v", err))
	}
	_ = initResult
	...
	if err != nil {
		log.Warn("[domain][mcp] CallMCPTool call failed",
			zap.Int64("server_id", serverID),
			zap.String("tool_name", toolName),
			zap.Int64("duration_ms", durationMs),
			zap.Error(err))
	...
	log.Info("[domain][mcp] CallMCPTool succeeded",
		zap.Int64("server_id", serverID),
		zap.String("tool_name", toolName),
		zap.Int64("duration_ms", durationMs),
