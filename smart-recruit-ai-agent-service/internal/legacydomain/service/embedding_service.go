...
func (s *EmbeddingService) RebuildProvider(ctx context.Context) {
	...
	if _, ok := next.(UnavailableEmbeddingProvider); ok {
		logger.L().Warn("[domain][embedding] provider rebuilt: unavailable (check provider/model config)")
		return
	}
	logger.L().Info("[domain][embedding] provider rebuilt successfully")
}
...
func (s *EmbeddingService) EmbedObject(ctx context.Context, input EmbedObjectInput) (*model.AIEmbedding, error) {
	...
	log.Info("[domain][embedding] EmbedObject started",
	...
	if s == nil || s.repo == nil {
		log.Error("[domain][embedding] repository not configured")
	...
	if objectType == "" || input.ObjectID == 0 {
		log.Warn("[domain][embedding] missing object type or id")
	...
	if len(vector) == 0 {
		...
		if err != nil {
			log.Warn("[domain][embedding] provider call failed, falling back to unavailable",
				zap.String("object_type", objectType),
				zap.Uint64("object_id", input.ObjectID),
	...
	if err := validateEmbeddingVector(vector); err != nil {
		log.Error("[domain][embedding] invalid embedding vector", zap.Error(err))
		return nil, err
	}
	...
	log.Info("[domain][embedding] EmbedObject succeeded",
		zap.String("object_type", objectType),
		zap.Uint64("object_id", input.ObjectID),
		zap.String("text_hash", textHash),
...
func (s *EmbeddingService) searchWithMeta(ctx context.Context, input EmbeddingSearchInput) ([]EmbeddingSearchResult, SearchMeta, error) {
	...
	if !policy.SemanticRetrieval {
		log.Info("[domain][embedding] semantic retrieval disabled",
	...
	log.Info("[domain][embedding] Search started",
	...
	log.Info("[domain][embedding] query vector resolved",
	...
	log.Info("[domain][embedding] candidates loaded",
...
func (s *EmbeddingService) SearchObjects(ctx context.Context, input EmbeddingObjectSearchInput) ([]EmbeddingSearchResult, error) {
	...
	log.Info("[domain][embedding] SearchObjects started",
		zap.String("object_type", objectType),
	...
	finalResults := results[:limit]
	log.Info("[domain][embedding] SearchObjects finished",
		zap.Int("candidate_count", len(rows)),
