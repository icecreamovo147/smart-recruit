...
export const listBillingOrders = (): Promise<{ orders: BillingOrder[]; total: number }> => request.get('/api/v1/hr/billing/orders')
export const createBillingOrder = (priceVersionId: number, orderType: string): Promise<{ order: BillingOrder }> => request.post('/api/v1/hr/billing/orders', { price_version_id: priceVersionId, order_type: orderType, idempotency_key: crypto.randomUUID() })
export const payBillingOrder = (orderNo: string, scene: 'desktop' | 'wap'): Promise<{ redirect_url: string; payment_environment: string }> => request.post(`/api/v1/hr/billing/orders/${orderNo}/pay`, { scene })
