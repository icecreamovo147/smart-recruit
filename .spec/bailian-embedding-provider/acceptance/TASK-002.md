# TASK-002 Acceptance - 新增 Embedding Provider/Model 的 Model 与 Repository
...
- 验证：新增 model 字段与 SDD 一致。
- 验证：Repository 支持读取启用 provider/model 和默认模型。
- 验证：SetDefault 相关能力具备事务或明确由 service 事务包裹。
- 验证：go test 通过。
...
- 现有 repository 单测。
- Agent Skill 和 ai_embeddings repository 行为。

## 6. 完成后必须输出的报告内容

- TASK-002 是否完成，以及完成摘要。
- 实际修改文件列表。
- 执行过的命令、结果和失败原因。
- 未执行的命令及原因。
- 是否触碰公共模块或越界文件。
