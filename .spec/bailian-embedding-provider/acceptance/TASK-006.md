# TASK-006 Acceptance - 接入 Services 初始化流程

## 1. 任务目标

将 services.go 中硬编码的 UnavailableEmbeddingProvider 替换为 Factory 构建结果。
...
- 验证：无配置时服务仍可启动。
- 验证：有默认模型时注入真实 provider。
- 验证：DebugSemanticRetrieval 可读取 provider 状态。
- 验证：go test 通过。
...
- logic-grpc-service 全量测试。
- 语义召回调试接口。

## 6. 完成后必须输出的报告内容

- TASK-006 是否完成，以及完成摘要。
- 实际修改文件列表。
- 执行过的命令、结果和失败原因。
- 未执行的命令及原因。
- 是否触碰公共模块或越界文件。
