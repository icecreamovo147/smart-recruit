# TASK-011 Acceptance - 新增 Embedding Backfill 命令

## 1. 任务目标

提供历史 Agent Skill 和 AI Memory 批量生成 embedding 的运维命令。

## 2. 自动化测试检查项

- 执行 `cd logic-grpc-service && go test ./...`，结果必须通过；若项目暂未配置该命令，必须在报告中说明。
- 验证：dry-run 不写入数据且统计正确。
- 验证：force=false 跳过已有 ready。
- 验证：force=true 重新写入。
- 验证：go test 通过。
