...
同时，当前核心业务模型、GORM 仓储、领域/应用服务、消息、OSS、AI、email、migration 仍主要集中在 `smart-recruit-domain-go`。各微服务通过 `cmd/*/main.go` 装配 `smart-recruit-domain-go/repository` 与 `smart-recruit-domain-go/service`，再通过各自 `internal/runtime` 暴露 gRPC 服务。`smart-recruit-proto` 是当前唯一 protobuf 契约来源，`smart-recruit-platform-go` 承载配置、Nacos、日志、健康检查、metrics、trace、metadata、gRPC helper 等平台能力。
...
- 将后端从“共享 `smart-recruit-domain-go` 领域内核”演化为更标准、规范的“服务自治微服务 + DDD”架构。
...
- 在迁移过程中逐步收缩 `smart-recruit-domain-go` 的职责，使其最终仅保留真正跨上下文共享、稳定、无业务归属争议的 shared kernel / platform-adjacent 能力。
- 在所有服务自治迁移、共享业务代码清理和 shared kernel 范围收敛完成后，将 `smart-recruit-domain-go` 作为本功能点最后的重命名任务迁移为 `smart-recruit-commons`。
...
- 本 SPEC 不要求把 `smart-recruit-domain-go` 的能力全部删除；迁移后它的 shared kernel 内容会保留，但最终目录与 Go module 名称应在最后阶段重命名为 `smart-recruit-commons`。
...
- FR-016: 每迁移一个服务，必须减少该服务对 `smart-recruit-domain-go/service`、`smart-recruit-domain-go/repository`、共享业务 model 的直接依赖，并记录剩余依赖是否为临时兼容、shared kernel 或待迁移债务。
...
- FR-027: 所有服务迁移、共享业务代码清理、shared kernel 范围确认和依赖收敛完成后，本功能点必须以最终重命名 TASK 将 `smart-recruit-domain-go` 重命名为 `smart-recruit-commons`，并同步更新 Go module path、import path、`go.work`、构建脚本、部署引用、文档和边界检查。
...
- CR-010: 当前 `smart-recruit-domain-go/config/config.example.yaml` 作为本地配置模板的用途必须保持；真实 `config.yaml` 不得重新进入 git 跟踪。
- CR-011: 在最终重命名前，所有仍引用 `smart-recruit-domain-go` 的服务必须明确分类为待迁移业务依赖、允许保留的 shared kernel 依赖或临时兼容债务；重命名后不得残留旧 module/import path。
...
- AC-003: SPEC 和 SDD 明确当前架构事实：已拆出独立服务根，但核心业务 model/repository/service 仍集中在 `smart-recruit-domain-go`。
- AC-004: SPEC 和 SDD 明确目标架构：每个微服务拥有本地 `domain`、`application`、`infrastructure`、`interfaces`、`runtime` 分层。
- AC-005: SPEC 和 SDD 明确 `smart-recruit-domain-go` 的收缩方向和允许保留的 shared kernel 范围。
...
- AC-012: SPEC 和 SDD 明确最终重命名策略：`smart-recruit-domain-go` 在 shared kernel 收敛后作为本功能点最后阶段重命名为 `smart-recruit-commons`。

## 12. Out of Scope

- 生成 TASKS 与 Harness 文件。
- 实施任何服务迁移。
- 在 `draft-spec-sdd` 阶段删除、移动或重命名 `smart-recruit-domain-go` 业务代码。
...
- ARC-004: `smart-recruit-domain-go` 可以阶段性继续被多个服务依赖，但每个服务完成迁移后必须减少或消除对其中业务 `model/repository/service` 的直接依赖。
...
- ARC-007: `smart-recruit-domain-go` 最终保留 shared kernel，不保留具体业务 `model/repository/service`。
...
- ARC-016: 本功能点最后必须执行 `smart-recruit-domain-go -> smart-recruit-commons` 的重命名 TASK，且该 TASK 只能在业务代码迁出、shared kernel 范围收敛、旧 import path 可清零后开始。
