# Implement Single Task Prompt (Followups)

你要执行 `.spec/skill-memory-ranking-followups/` 下的单个 TASK。不要同时执行多个 TASK。
...
- 不得修改 TASK-005 / TASK-006 / TASK-007 范围内的现有代码（除非本 TASK 明确允许）。
...
3. 运行 `.spec/skill-memory-ranking-followups/scripts/check-task-scope.sh <TASK-ID>` 并核对越界风险。
4. 运行 `.spec/skill-memory-ranking-followups/scripts/agent-check.sh` 并确认通过。
5. 输出完成报告：
   - TASK 编号和名称；
   - 修改文件；
   - 自测命令和结果；
...
- 需要修改 TASK-001..008 已落地的算法公式 / proto 字段 / 函数签名。
