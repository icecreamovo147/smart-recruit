...
// --- typed input/output structs (DEV parity) ---
...
// NewCandidateADKTools creates the six candidate tools as Eino BaseTools.
// Business logic lives in ToolRunner.Execute; per-request owner ID is read from
// context via WithOwnerID (so tools can be cached across requests).
func NewCandidateADKTools(executor ToolRunner) ([]tool.BaseTool, error) {
	if executor == nil {
	...
	recommendByResumeTool, err := utils.InferTool(
		...
		"基于当前候选人上传简历的解析文本和系统内所有岗位，生成 3-5 个推荐岗位，包含匹配理由、不足点和建议投递优先级。如果候选人没有上传简历或简历解析无效，返回提示信息。每个推荐岗位包含 has_applied 标记",
...
// FilterCandidateADKToolsByName filters tools by allowlist.
// Empty allowlist returns tools unchanged (DEV candidate runtime semantics).
func FilterCandidateADKToolsByName(tools []tool.BaseTool, allowlist []string) []tool.BaseTool {
	if len(tools) == 0 || len(allowlist) == 0 {
		return tools
	}
	return FilterADKToolsByName(tools, allowlist)
}

// FilterCandidateToolInfosByName filters ToolInfo schemas by allowlist.
// Empty allowlist returns tools unchanged (DEV candidate runtime semantics).
func FilterCandidateToolInfosByName(tools []*schema.ToolInfo, allowlist []string) []*schema.ToolInfo {
	if len(tools) == 0 || len(allowlist) == 0 {
		return tools
	}
