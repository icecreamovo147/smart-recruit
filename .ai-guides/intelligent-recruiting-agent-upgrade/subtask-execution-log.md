...
| T005 | ADK tools for resume profile and candidate match workflows | passed | feature/intelligent-recruiting-agent-upgrade | - | PASS | 0 | `go test ./ai` passed; focused AI/service tests passed; `go test ./...` in `logic-grpc-service` passed with escalation for httptest bind | Rollback point pending commit. |
