...
| T017 | MCP policy management UI and trace exposure | passed | feature/intelligent-recruiting-agent-upgrade | - | PASS | 1 | `pnpm --filter hr-frontend typecheck` passed; `go test ./...` passed in `logic-grpc-service` and `web-gin-service`; `git diff --check` passed | Reviewer PASS after gRPC forwarding fix; rollback commit pending. |
