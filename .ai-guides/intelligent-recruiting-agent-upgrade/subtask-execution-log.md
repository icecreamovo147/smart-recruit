...
| T006 | Structured planner output model | passed | feature/intelligent-recruiting-agent-upgrade | - | PASS | 0 | `go test ./ai -run 'TestRecruitingPlanner'` passed; `go test ./service -run TestBuildToolCallingMessagesNoToolsForGreeting` passed; `go test ./...` in `logic-grpc-service` passed with escalation for httptest bind | Rollback point pending commit. |
| T007 | Agent Run plan, evidence, decision, Skill, and Memory persistence | pending | integration/agent-platform | - | - | 0 | `go test ./...` in `logic-grpc-service` focused Agent Run tests | Extends run/step persistence for plan, evidence chain, decision, selected skills/memories. |
| T008 | HR resume profile and match evaluation views | pending | integration/agent-platform | - | - | 0 | `pnpm --filter hr-frontend typecheck`; frontend tests if logic is added | Adds HR inspection/rerun UI under candidate/application context. |
| T009 | HR Agent trace UI upgrade | pending | integration/agent-platform | - | - | 0 | `pnpm --filter hr-frontend typecheck`; frontend tests if logic is added | Displays plan, capability selection, skill/memory selection, evidence, decisions, final answer. |
| T010 | Agent Skill metadata model, API, and validation | pending | integration/agent-platform | - | - | 0 | `go test ./...` in `logic-grpc-service`; `go test ./...` in `web-gin-service` if API changes are included | Adds governed metadata and required-capability validation. |
| T011 | Agent Skill planner/runtime selection integration | pending | integration/agent-platform | - | - | 0 | `go test ./...` in `logic-grpc-service` focused selector/planner tests | Uses metadata to influence selected skills, required tools, and output schema. |
| T012 | HR Agent Skill editor metadata controls | pending | integration/agent-platform | - | - | 0 | `pnpm --filter hr-frontend typecheck`; frontend tests if logic is added | Extends existing Agent Skill page without breaking current canvas flow. |
