...
func TestApplyEnvOverrides(t *testing.T) {
	...
	t.Setenv("MCP_DEFAULT_TIMEOUT_SECONDS", "11")
	t.Setenv("MCP_DEFAULT_MAX_RETRIES", "2")
	t.Setenv("MCP_ALLOW_STDIO", "true")
	t.Setenv("MCP_ALLOWED_STDIO_COMMANDS", "npx,node")
	t.Setenv("MCP_ALLOWED_URL_HOSTS", "api.example.com,tools.example.com")
	t.Setenv("MCP_BLOCK_PRIVATE_NETWORK", "false")
	t.Setenv("MCP_MAX_TIMEOUT_SECONDS", "99")
	...
	if cfg.MCP.DefaultTimeoutSeconds != 11 || cfg.MCP.DefaultMaxRetries != 2 || !cfg.MCP.AllowStdio || cfg.MCP.MaxTimeoutSeconds != 99 {
		t.Fatalf("unexpected mcp scalar overrides: %+v", cfg.MCP)
	}
	if strings.Join(cfg.MCP.AllowedStdioCommands, ",") != "npx,node" {
		t.Fatalf("unexpected mcp stdio commands: %v", cfg.MCP.AllowedStdioCommands)
	}
	if strings.Join(cfg.MCP.AllowedURLHosts, ",") != "api.example.com,tools.example.com" {
		t.Fatalf("unexpected mcp url hosts: %v", cfg.MCP.AllowedURLHosts)
	}
