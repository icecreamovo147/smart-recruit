...
// wrapAIError wraps an AI client error as a gRPC Internal status with "ai:" prefix.
// If err is already a gRPC status error, it's returned as-is.
func wrapAIError(err error) error {
	...
	return status.Errorf(codes.Internal, "ai: %v", err)
}
...
func maskPhone(phone string) string {
	...
	last := string(runes[len(runes)-4:])
	if len(runes) <= 7 {
		return "****" + last
	}
	return string(runes[:3]) + "****" + last
}
...
// actionRule defines a recognized intent and the target application status.
type actionRule struct {
	Status  int32
	Phrases []string
}

var applicationActionRules = []actionRule{
	{Status: 3, Phrases: []string{"标记为淘汰", "设为淘汰", "改为淘汰", "淘汰这个候选人", "淘汰该候选人", "淘汰他", "淘汰她", "把他淘汰", "把她淘汰", "帮我淘汰", "不通过", "拒绝这个候选人", "拒绝该候选人"}},
	{Status: 2, Phrases: []string{"标记为通过", "设为通过", "改为通过", "通过这个候选人", "通过该候选人", "通过他的申请", "通过她的申请", "让他通过", "让她通过", "帮我通过", "同意录用", "录用这个候选人", "录用该候选人", "进入下一轮"}},
}

func detectApplicationAction(message string) (int32, bool) {
	text := strings.TrimSpace(message)
	for _, rule := range applicationActionRules {
...
func extractAndStoreResumeText(ctx context.Context, resumeID int64, fileType, ossKey string, ossClient *oss.Client, resumeRepo *repository.ResumeRepo) (string, error) {
	start := time.Now()
...
func loadOrRefreshResumeText(ctx context.Context, detail *repository.ApplicationDetailRow, ossClient *oss.Client, resumeRepo *repository.ResumeRepo) resumeTextResult {
...
func buildToolCallingMessages(actx *AgentContext, currentMessage string) []*schema.Message {
	...
	systemPrompt := "你是智能招聘系统的 AI 数据助手。你可以使用提供的工具查询真实的招聘数据来回答 HR 的问题。你只能回答与招聘系统相关的问题，如果用户询问与招聘无关的内容（如产品评测、技术算法、生活建议等），必须礼貌拒绝并引导回到招聘话题。\n\n" +
		...
		"相关长期记忆：\n" + memorySection + "\n\n" +
		"重要规则：\n" +
		"1. 只有当用户明确询问岗位、候选人、投递、简历分析、招聘统计、趋势或状态变更等招聘数据时，才调用工具；普通问候、感谢、能力介绍、使用说明、闲聊和澄清问题禁止调用工具，应直接简洁回复\n" +
		"2. 当前用户消息优先级最高。不得仅凭会话摘要、长期记忆或历史对话为当前简短问候补全查询意图，也不得擅自添加用户没有提到的岗位关键词、候选人姓名或时间范围\n" +
		"3. 必须基于工具返回的真实数据回答，不得编造任何数据\n" +
		"4. 工具实时查询结果优先于摘要和长期记忆；如果记忆与工具结果冲突，以工具结果为准\n" +
		"5. 长期记忆可能过期，涉及实时数据（统计、状态、最新简历等）必须调用工具查询\n" +
		"6. 搜索候选人时，姓名采用精确匹配。只有工具返回空列表时才能说未找到\n" +
		"7. 当 total <= 20 时直接列出全部结果。只有 total > 50 时才建议筛选\n" +
		"8. 回答要精炼、专业、中文输出。分析候选人时只给核心匹配点和风险点，每条不超过两行。列出多条数据时每条记录控制在 1-2 句话。\n" +
		"9. 如果问题涉及多个方面，请依次调用相关工具\n" +
		"10. 分析候选人简历或岗位匹配度时，必须调用 get_candidate_detail，并以工具返回的 resume_text 和岗位信息为主要依据；如果 resume_text 为空，要明确说明无法充分基于简历正文判断，不得编造经历\n" +
		"11. 当 HR 要求通过、淘汰、拒绝、录用、进入下一轮等状态变更时，必须调用 propose_application_status_update 生成待确认动作，回复中请 HR 确认，不要声称已经更新；状态变更必须请求 HR 确认后才能执行\n" +
		"12. 对于不明确的问题，可以请求 HR 补充信息"
