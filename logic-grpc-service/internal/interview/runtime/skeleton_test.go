...
func TestNewDescriptorIsUnroutedInterviewService(t *testing.T) {
...
func TestDescriptorDocumentsNoRuntimeSideEffects(t *testing.T) {
	...
	for _, required := range []string{
		...
		"gateway traffic is not routed to interview-service",
	} {
...
func TestDescriptorListsInterviewExtractedAPIs(t *testing.T) {
	descriptor, err := NewDescriptor()
	if err != nil {
		t.Fatalf("NewDescriptor() error = %v", err)
	}
...
func TestValidateRejectsTrafficEnabledSkeleton(t *testing.T) {
	...
	if err := Validate(descriptor); err == nil {
		t.Fatal("Validate accepted traffic-enabled interview skeleton")
	}
}
