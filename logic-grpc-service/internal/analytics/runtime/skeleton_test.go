package runtime

import (
	"strings"
	"testing"

	"logic-grpc-service/internal/platform/servicebinary"
)

func TestNewDescriptorIsUnroutedAnalyticsService(t *testing.T) {
	descriptor, err := NewDescriptor()
	if err != nil {
		t.Fatalf("NewDescriptor() error = %v", err)
	}
	if descriptor.Unit.Name != ServiceName {
		t.Fatalf("unit name = %q, want %q", descriptor.Unit.Name, ServiceName)
	}
	if descriptor.Unit.Role != servicebinary.RoleService {
		t.Fatalf("unit role = %q, want %q", descriptor.Unit.Role, servicebinary.RoleService)
	}
	if descriptor.TrafficEnabled {
		t.Fatal("analytics skeleton must not enable traffic")
	}
	if descriptor.CutoverMode != "none" {
		t.Fatalf("cutover mode = %q, want none", descriptor.CutoverMode)
	}
	if descriptor.ProjectionMode != "domain-event-projection-read-models" {
		t.Fatalf("projection mode = %q", descriptor.ProjectionMode)
	}
}

func TestDescriptorDocumentsProjectionReadModels(t *testing.T) {
	descriptor, err := NewDescriptor()
	if err != nil {
		t.Fatalf("NewDescriptor() error = %v", err)
	}
	notes := strings.Join(descriptor.Notes, "\n")
	for _, required := range []string{
		"default execution does not bind a network listener",
		"projection/read-model ports",
		"does not introduce transactional service-read adapters",
		"QueryAuthAuditLogs remains Identity-owned",
	} {
		if !strings.Contains(notes, required) {
			t.Fatalf("descriptor notes missing %q: %s", required, notes)
		}
	}
}

func TestDescriptorListsAnalyticsExtractedAPIs(t *testing.T) {
	descriptor, err := NewDescriptor()
	if err != nil {
		t.Fatalf("NewDescriptor() error = %v", err)
	}
	apis := strings.Join(descriptor.ExtractedAPIs, "\n")
	for _, required := range []string{
		"AdminService.GetDashboardReport",
		"AdminService.GetFunnelReport",
		"AdminService.GetTimeInStageReport",
		"AdminService.GetInterviewOfferMetrics",
	} {
		if !strings.Contains(apis, required) {
			t.Fatalf("descriptor extracted APIs missing %q: %s", required, apis)
		}
	}
	if strings.Contains(apis, "QueryAuthAuditLogs") {
		t.Fatalf("analytics runtime must not claim Identity-owned QueryAuthAuditLogs: %s", apis)
	}
}

func TestValidateRejectsTrafficEnabledSkeleton(t *testing.T) {
	descriptor, err := NewDescriptor()
	if err != nil {
		t.Fatalf("NewDescriptor() error = %v", err)
	}
	descriptor.TrafficEnabled = true
	if err := Validate(descriptor); err == nil {
		t.Fatal("Validate accepted traffic-enabled analytics skeleton")
	}
}
