...
func NewDescriptor() (Descriptor, error) {
	...
	descriptor := Descriptor{
		...
		Notes: []string{
			...
			"OfferService is registered only through explicit runtime registration",
			"does not start offer lifecycle workers or consumers",
			"does not mutate offers, notifications, events, or application lifecycle state by default",
			"gateway traffic is not routed to offer-service in this TASK",
			"ready for later controlled offer gateway cutover with rollback evidence",
		},
	}
