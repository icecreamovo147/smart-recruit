...
func TestNewDescriptorIsUnroutedNotificationService(t *testing.T) {
	...
	if descriptor.TrafficEnabled {
		t.Fatal("notification skeleton must not enable traffic")
	}
...
func TestDescriptorDocumentsNoRuntimeSideEffects(t *testing.T) {
	...
	for _, required := range []string{
		...
		"does not start notification consumers",
...
func TestValidateRejectsTrafficEnabledSkeleton(t *testing.T) {
	...
	if err := Validate(descriptor); err == nil {
		t.Fatal("Validate accepted traffic-enabled notification skeleton")
	}
}
