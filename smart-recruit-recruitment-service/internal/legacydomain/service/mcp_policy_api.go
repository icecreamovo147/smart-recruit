...
func (s *MCPService) ListMCPToolPolicies(ctx context.Context, req *pb.ListMCPToolPoliciesRequest) (*pb.ListMCPToolPoliciesResponse, error) {
	...
	log.Info("[domain][mcp_policy] ListMCPToolPolicies started", zap.Int64("server_id", req.GetServerId()))
	...
	if err != nil {
		log.Error("[domain][mcp_policy] list MCP tool policies failed", zap.Error(err))
	...
	log.Info("[domain][mcp_policy] ListMCPToolPolicies succeeded", zap.Int("total", int(total)), zap.Int("returned", len(list)))
...
func (s *MCPService) CreateMCPToolPolicy(ctx context.Context, req *pb.CreateMCPToolPolicyRequest) (*pb.MCPToolPolicyResponse, error) {
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][mcp_policy] CreateMCPToolPolicy started",
	...
	if err != nil {
		log.Warn("[domain][mcp_policy] CreateMCPToolPolicy validation failed", zap.Error(err))
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
	if _, err := s.mcpRepo.GetServerByID(ctx, policy.ServerID); err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Warn("[domain][mcp_policy] CreateMCPToolPolicy server not found")
			return nil, status.Error(codes.NotFound, "MCP server not found")
		}
		log.Error("[domain][mcp_policy] CreateMCPToolPolicy get server failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get MCP server failed")
	}
	if err := s.mcpRepo.CreateToolPolicy(ctx, policy); err != nil {
		log.Error("[domain][mcp_policy] create MCP tool policy failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "create MCP tool policy failed")
	}
	log.Info("[domain][mcp_policy] CreateMCPToolPolicy succeeded",
...
func (s *MCPService) UpdateMCPToolPolicy(ctx context.Context, req *pb.UpdateMCPToolPolicyRequest) (*pb.MCPToolPolicyResponse, error) {
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][mcp_policy] UpdateMCPToolPolicy started", zap.Int64("policy_id", req.GetId()))
	if req.GetId() <= 0 {
		log.Warn("[domain][mcp_policy] UpdateMCPToolPolicy invalid id")
		return nil, status.Error(codes.InvalidArgument, "id is required")
	}
	existing, err := s.mcpRepo.GetToolPolicyByID(ctx, req.GetId())
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			log.Warn("[domain][mcp_policy] UpdateMCPToolPolicy policy not found")
			return nil, status.Error(codes.NotFound, "MCP tool policy not found")
		}
		log.Error("[domain][mcp_policy] get MCP tool policy failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get MCP tool policy failed")
	}
	policy, err := buildMCPToolPolicyFromUpdate(existing, req)
	if err != nil {
		log.Warn("[domain][mcp_policy] UpdateMCPToolPolicy validation failed", zap.Error(err))
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
	if _, err := s.mcpRepo.GetServerByID(ctx, policy.ServerID); err != nil {
		if err == gorm.ErrRecordNotFound {
		...
		log.Error("[domain][mcp_policy] get MCP server failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get MCP server failed")
	}
	if err := s.mcpRepo.UpdateToolPolicy(ctx, policy); err != nil {
		log.Error("[domain][mcp_policy] update MCP tool policy failed", zap.Error(err))
	...
	if err != nil {
		log.Error("[domain][mcp_policy] get updated MCP tool policy failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "get updated MCP tool policy failed")
	}
	log.Info("[domain][mcp_policy] UpdateMCPToolPolicy succeeded",
...
func (s *MCPService) DeleteMCPToolPolicy(ctx context.Context, req *pb.DeleteMCPToolPolicyRequest) (*pb.CommonResponse, error) {
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][mcp_policy] DeleteMCPToolPolicy started", zap.Int64("policy_id", req.GetId()))
	if req.GetId() <= 0 {
		log.Warn("[domain][mcp_policy] DeleteMCPToolPolicy invalid id")
		return nil, status.Error(codes.InvalidArgument, "id is required")
	}
	if err := s.mcpRepo.DeleteToolPolicy(ctx, req.GetId()); err != nil {
		log.Error("[domain][mcp_policy] delete MCP tool policy failed", zap.Error(err))
		return nil, status.Error(codes.Internal, "delete MCP tool policy failed")
	}
	log.Info("[domain][mcp_policy] DeleteMCPToolPolicy succeeded", zap.Int64("policy_id", req.GetId()))
