...
func (s *RecruitingIntelligenceService) GetResumeProfile(ctx context.Context, req *pb.GetResumeProfileRequest) (*pb.GetResumeProfileResponse, error) {
	...
	log.Info("[domain][recruiting_intelligence] GetResumeProfile started",
	...
	if req.GetApplicationId() <= 0 && req.GetResumeId() <= 0 && req.GetProfileId() == 0 {
		log.Warn("[domain][recruiting_intelligence] GetResumeProfile validation failed: missing identifier")
	...
	if err := s.validateResumeIdentifierConsistency(ctx, req.GetResumeId(), req.GetProfileId(), req.GetApplicationId()); err != nil {
		log.Warn("[domain][recruiting_intelligence] GetResumeProfile consistency check failed", zap.Error(err))
	...
	if err := s.authorizeApplicationRead(ctx, req.GetStaffUserId(), req.GetApplicationId(), req.GetResumeId(), req.GetProfileId()); err != nil {
		log.Warn("[domain][recruiting_intelligence] GetResumeProfile authorization failed", zap.Error(err))
	...
	if resp != nil {
		log.Warn("[domain][recruiting_intelligence] GetResumeProfile resolution failed", zap.String("msg", resp.GetMsg()))
		return resp, nil
	}
	log.Info("[domain][recruiting_intelligence] GetResumeProfile succeeded",
...
func (s *RecruitingIntelligenceService) ParseResumeProfile(ctx context.Context, req *pb.ParseResumeProfileRequest) (*pb.GetResumeProfileResponse, error) {
	started := time.Now()
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][recruiting_intelligence] ParseResumeProfile started",
		zap.Int64("staff_user_id", req.GetStaffUserId()),
		zap.Int64("application_id", req.GetApplicationId()),
		zap.Int64("resume_id", req.GetResumeId()),
	)
	if req.GetApplicationId() <= 0 && req.GetResumeId() <= 0 {
		log.Warn("[domain][recruiting_intelligence] ParseResumeProfile validation failed: missing identifier")
	...
	if err := s.validateResumeIdentifierConsistency(ctx, req.GetResumeId(), 0, req.GetApplicationId()); err != nil {
		log.Warn("[domain][recruiting_intelligence] ParseResumeProfile consistency check failed", zap.Error(err))
		return &pb.GetResumeProfileResponse{Code: errs.ErrBadRequest, Msg: err.Error()}, nil
	}
	if err := s.authorizeApplicationRead(ctx, req.GetStaffUserId(), req.GetApplicationId(), req.GetResumeId(), 0); err != nil {
		log.Warn("[domain][recruiting_intelligence] ParseResumeProfile authz failed", zap.Error(err))
		return &pb.GetResumeProfileResponse{Code: errs.ErrForbidden, Msg: err.Error()}, nil
	}
	if err := s.authorizePermission(ctx, req.GetStaffUserId(), authz.PermAIHRUse); err != nil {
		log.Warn("[domain][recruiting_intelligence] ParseResumeProfile permission denied", zap.Error(err))
		return &pb.GetResumeProfileResponse{Code: errs.ErrForbidden, Msg: err.Error()}, nil
	}
	resumeID, err := s.resolveResumeID(ctx, req.GetResumeId(), req.GetApplicationId())
	if err != nil {
		log.Warn("[domain][recruiting_intelligence] ParseResumeProfile resolveResumeID failed", zap.Error(err))
		return &pb.GetResumeProfileResponse{Code: errs.ErrBadRequest, Msg: err.Error()}, nil
	}
	log.Info("[domain][recruiting_intelligence] ParseResumeProfile calling profileSvc.ParseResume",
		zap.Int64("resolved_resume_id", resumeID))
	snapshot, err := s.profileSvc.ParseResume(ctx, resumeID)
	if err != nil {
		log.Warn("[domain][recruiting_intelligence] ParseResumeProfile parse failed",
			zap.Int64("duration_ms", time.Since(started).Milliseconds()),
			zap.Error(err))
		return &pb.GetResumeProfileResponse{Code: businessCodeForResumeProfileError(err), Msg: err.Error()}, nil
	}
	log.Info("[domain][recruiting_intelligence] ParseResumeProfile succeeded",
		zap.Int64("duration_ms", time.Since(started).Milliseconds()),
...
func (s *RecruitingIntelligenceService) EvaluateCandidateMatch(ctx context.Context, req *pb.EvaluateCandidateMatchRequest) (*pb.GetCandidateMatchEvaluationResponse, error) {
	started := time.Now()
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][recruiting_intelligence] EvaluateCandidateMatch started",
		zap.Int64("staff_user_id", req.GetStaffUserId()),
		zap.Int64("application_id", req.GetApplicationId()),
	...
	if err := s.authorizeApplication(ctx, req.GetStaffUserId(), req.GetApplicationId(), authz.PermApplicationRead); err != nil {
		log.Warn("[domain][recruiting_intelligence] EvaluateCandidateMatch authz failed", zap.Error(err))
		return &pb.GetCandidateMatchEvaluationResponse{Code: errs.ErrForbidden, Msg: err.Error()}, nil
	}
	if err := s.authorizePermission(ctx, req.GetStaffUserId(), authz.PermAIHRUse); err != nil {
		log.Warn("[domain][recruiting_intelligence] EvaluateCandidateMatch permission denied", zap.Error(err))
		return &pb.GetCandidateMatchEvaluationResponse{Code: errs.ErrForbidden, Msg: err.Error()}, nil
	}
	...
	if err != nil {
		log.Warn("[domain][recruiting_intelligence] EvaluateCandidateMatch failed",
			zap.Int64("duration_ms", time.Since(started).Milliseconds()),
			zap.Error(err))
		return &pb.GetCandidateMatchEvaluationResponse{Code: businessCodeForCandidateMatchError(err), Msg: err.Error()}, nil
	}
	log.Info("[domain][recruiting_intelligence] EvaluateCandidateMatch succeeded",
		zap.Int64("duration_ms", time.Since(started).Milliseconds()),
...
func (s *RecruitingIntelligenceService) GetCandidateMatchEvaluation(ctx context.Context, req *pb.GetCandidateMatchEvaluationRequest) (*pb.GetCandidateMatchEvaluationResponse, error) {
	started := time.Now()
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][recruiting_intelligence] GetCandidateMatchEvaluation started",
		zap.Int64("staff_user_id", req.GetStaffUserId()),
		zap.Int64("application_id", req.GetApplicationId()),
	...
	if err := s.authorizeApplication(ctx, req.GetStaffUserId(), req.GetApplicationId(), authz.PermApplicationRead); err != nil {
		log.Warn("[domain][recruiting_intelligence] GetCandidateMatchEvaluation authz failed", zap.Error(err))
		return &pb.GetCandidateMatchEvaluationResponse{Code: errs.ErrForbidden, Msg: err.Error()}, nil
	}
	evaluation, err := s.resolveCandidateMatchEvaluation(ctx, req)
	if err != nil {
		log.Error("[domain][recruiting_intelligence] GetCandidateMatchEvaluation resolve failed", zap.Error(err))
		return nil, err
	}
	if evaluation == nil {
		log.Warn("[domain][recruiting_intelligence] GetCandidateMatchEvaluation not found",
			zap.Int64("duration_ms", time.Since(started).Milliseconds()))
	...
	if err != nil {
		log.Error("[domain][recruiting_intelligence] GetCandidateMatchEvaluation GetSnapshot failed", zap.Error(err))
		return nil, err
	}
	log.Info("[domain][recruiting_intelligence] GetCandidateMatchEvaluation succeeded",
		zap.Int64("duration_ms", time.Since(started).Milliseconds()),
		zap.Uint64("evaluation_id", snapshot.Evaluation.ID),
...
func (s *RecruitingIntelligenceService) CompareCandidatesForJob(ctx context.Context, req *pb.CompareCandidatesForJobRequest) (*pb.CompareCandidatesForJobResponse, error) {
	started := time.Now()
	log := logger.GetRequestLogger(ctx)
	log.Info("[domain][recruiting_intelligence] CompareCandidatesForJob started",
		zap.Int64("staff_user_id", req.GetStaffUserId()),
		zap.Int64("job_id", req.GetJobId()),
	)
	if err := s.authorizeJob(ctx, req.GetStaffUserId(), req.GetJobId(), authz.PermApplicationRead); err != nil {
		log.Warn("[domain][recruiting_intelligence] CompareCandidatesForJob authz failed", zap.Error(err))
	...
	if err != nil {
		log.Error("[domain][recruiting_intelligence] CompareCandidatesForJob ListCurrentByJob failed", zap.Error(err))
		return nil, err
	}
	...
	if err != nil {
		log.Error("[domain][recruiting_intelligence] CompareCandidatesForJob ListLatestByApplicationIDs failed", zap.Error(err))
		return nil, err
	}
	...
	log.Info("[domain][recruiting_intelligence] CompareCandidatesForJob succeeded",
		zap.Int64("duration_ms", time.Since(started).Milliseconds()),
		zap.Int64("job_id", req.GetJobId()),
