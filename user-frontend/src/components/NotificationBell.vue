...
const pollNotifications = async () => {
  ...
  try {
    ...
    const nextUnreadCount = Number(summary.unread) || 0
    unreadCount.value = nextUnreadCount
    if (!notificationBaselineReady && !unreadSummaryShown && nextUnreadCount > 0) {
      unreadSummaryShown = true
      void playSound()
      showUnreadSummaryNotification(nextUnreadCount)
    }
...
const showUnreadSummaryNotification = (count: number) => {
  ElNotification({
    title: '未读消息提醒',
    message: `你有 ${count} 条未读消息未处理`,
    position: 'top-right',
    duration: 6000,
    type: 'info',
  })
