...
func TestIdentityAdminClientRoutesRBACAndAuditToIdentity(t *testing.T) {
	logic := &recordingAdminClient{name: "logic"}
	identity := &recordingAdminClient{name: "identity"}
	client := newIdentityAdminClient(logic, identity)
	...
	if len(logic.calls) != 0 {
		t.Fatalf("logic admin client received identity-owned calls: %v", logic.calls)
	}
