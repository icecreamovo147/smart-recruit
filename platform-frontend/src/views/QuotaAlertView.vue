...
<template>
  <section class="console-page" v-loading="loading">
    <article class="surface-card table-surface">
      <div class="filter-toolbar filter-toolbar--wrap"><div class="filter-fields filter-fields--wrap"><el-input-number v-model="query.tenant_id" :min="1" :controls="false" placeholder="租户 ID" /><el-select v-model="query.status" clearable placeholder="告警状态"><el-option label="待处理" value="open" /><el-option label="已认领" value="acknowledged" /><el-option label="已解决" value="resolved" /></el-select><el-select v-model="query.metric_key" clearable placeholder="配额指标"><el-option v-for="(label, key) in metricLabels" :key="key" :label="label" :value="key" /></el-select></div><div class="filter-actions"><el-button type="primary" :icon="Search" @click="search">查询</el-button></div></div>
      ...
      <footer class="table-footer"><span>共 {{ total }} 条告警</span><el-pagination v-model:current-page="query.page" v-model:page-size="query.page_size" :total="total" layout="prev, pager, next, sizes" :page-sizes="[10,20,50,100]" @current-change="load" @size-change="load" /></footer>
    </article>
